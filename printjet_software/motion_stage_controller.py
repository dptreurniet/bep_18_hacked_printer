...
from ftdi_controller import GpioController
...
class MotionStageController():
    ...
    def move_to_position(self, position, log=False, debug=False):
        ...
        while True:
            ...
            possible_slopes = list(map(abs, [slope1-goal_slope, slope2-goal_slope, slope3-goal_slope]))
...
class Stage():
    ...
    def step(self, direction, step, disable_softstop=False, ftdi=False):
        if step:
            ...
            if ((new_pos <= self.range-self.soft_padding+1e-10 and new_pos >= self.soft_padding-1e-10) or
                ...
                if not ftdi:
                    ...
                    self.pulse_step_pin(direction)
                else:
                    self.step_ftdi(direction)
    ...
    def step_ftdi(self, direction):
        pass



    def pulse_step_pin(self, direction):
